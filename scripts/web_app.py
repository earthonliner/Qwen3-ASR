...
说明：
  ...
  - macOS 上没有 vLLM，无法用真正的流式解码；0.6B + MPS 下该方案延迟约数秒，够课堂使用。
  - 模型推理用全局锁串行化，多人同时使用会排队。
"""
...
class ASREngine:
    def __init__(self, model_path: str, backend: str, device: str, dtype_str: str,
        ...
        if self.backend == "mlx":
            ...
            print(f"[engine] 后端：{self.backend}，设备：{self.device}，"
        ...
        if self.backend == "mlx":
            ...
            print(f"[engine] 正在加载 qwen-asr 模型：{model_path} ……")
            t0 = time.time()
    ...
    def transcribe(self, audio_16k: np.ndarray, language: Optional[str]) -> Tuple[str, str]:
        ...
        with self.lock:
            ...
            return self._transcribe_package(audio_16k, language)
    ...
    def _transcribe_mlx(self, audio_16k: np.ndarray, language: Optional[str]) -> Tuple[str, str]:
        ...
        try:
            ...
            result = self.model.generate(tmp_path, **gen_kwargs)
            return rat._parse_raw_asr(getattr(result, "text", "") or "", language)
        finally:
    ...
    def _transcribe_package(self, audio_16k: np.ndarray, language: Optional[str]) -> Tuple[str, str]:
        results = self.model.transcribe(
            audio=(audio_16k, SAMPLE_RATE), language=language, return_time_stamps=False,
        )
        r = results[0]
        return (r.language or language or "", r.text or "")
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument("--backend", type=str, default="auto",
                   ...
                   help="推理后端；auto 按模型名自动选择（含 mlx → mlx，-hf → transformers）。")
    p.add_argument("--device", type=str, default="auto", choices=["auto", "mps", "cpu", "cuda:0"],
                   help="运行设备。")
