...
#   2. 通过 Homebrew 安装系统依赖（ffmpeg、sox、portaudio、可选 miniforge）
#   3. 创建并激活一个独立的 conda / venv Python 3.12 环境
#   4. 安装 PyTorch（自带 MPS/Metal 支持）、qwen-asr 以及录音所需的 sounddevice
#   5. 可选：从 ModelScope 预先下载 0.6B 模型（适合中国大陆网络）
#
# 用法：
#   bash scripts/setup_mac.sh                 # 默认安装
...
如果你用 ModelScope 预下载了模型，加 --model 指向本地目录，例如：
    python scripts/record_and_transcribe.py --record --model ${MODEL_DIR}/Qwen3-ASR-0.6B

提示：如果直接从 HuggingFace 下载模型很慢，可先设置镜像：
